...
typedef int CLIENT_PRINT_TYPE;

// Global vars
extern int MSGID_ShowMenu;
extern int MSGID_HLTV;
extern int MSGID_TextMsg;
extern int MSGID_SayText;
...
void UTIL_ClientPrint(edict_t *pEntity, CLIENT_PRINT_TYPE msg_dest, const char *msg, ...);
...
// Inline
inline bool IsAlive(const edict_t *pEdict)
{
	return ((pEdict->v.deadflag == DEAD_NO) && (pEdict->v.health > 0));
}

inline bool IsPlayer(const edict_t *pEdict)
{
	int temp = ENTINDEX(pEdict);
	return  ((1 <= temp) && (temp <= gpGlobals->maxClients));
}

inline bool IsPlayer(const int index)
{
	return  ((1 <= index) && (index <= gpGlobals->maxClients));
}
